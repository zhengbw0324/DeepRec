...
def masked_mean(tensor: torch.Tensor, mask: Optional[torch.Tensor], item_mask: Optional[torch.Tensor], dim: int = None) -> torch.Tensor:
    if item_mask is None:
        if mask is None:
            return tensor.mean(axis=dim)
        return (tensor * mask).sum(axis=dim) / (mask.sum(axis=dim) + 1e-5)
    else:
        assert mask is None, "mask and item_mask cannot be used together"
        return (tensor * item_mask).sum(axis=dim) / (item_mask.sum(axis=dim) + 1e-5)
